...
import type { Friend, FriendRequest } from '../js/store/types/friends.type';
...
export const updateFriendRequest = async (
  ...
  if (error) {
    console.error(
      'An error occurred trying to update the friend request',
      error.message
    );
    throw new Error(error.message);
  }
...
export const getFriendRequests = async (userId: User['id']) => {
  const { data, error } = await supabase
    .from('friendships')
    .select(`*,profile:profiles!friendships_requester_id_fkey1(*)`)
